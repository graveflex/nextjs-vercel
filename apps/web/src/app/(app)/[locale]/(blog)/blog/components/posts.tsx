...
async function fetchPostData({ draft, locale, searchParams }: LayoutProps) {
  const pagPage = (await searchParams).page;
  const filterPage = (await searchParams).filter;
  const sortPage = (await searchParams).sort;
  const searchPage = (await searchParams).search;
